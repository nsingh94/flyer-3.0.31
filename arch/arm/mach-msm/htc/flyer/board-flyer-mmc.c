...
static struct regulator *vreg_sdslot;	/* SD slot power */
...
static uint32_t flyer_sdslot_switchvdd(struct device *dev, unsigned int vdd)
{
	...
	for (i = 0; i < ARRAY_SIZE(mmc_vdd_table); i++) {
		if (mmc_vdd_table[i].mask == (1 << vdd)) {
			...
			regulator_set_voltage(vreg_sdslot, mmc_vdd_table[i].level*1000, mmc_vdd_table[i].level*1000);
			return 0;
		}
	}
...
int __init flyer_init_mmc(unsigned int sys_rev)
{
	...
	vreg_sdslot = regulator_get(0, "gp10");
	...
	gpio_set_value(FLYER_GPIO_EMMC_RST, 0);	/* this should not work!!! */
	udelay(100);
	gpio_set_value(FLYER_GPIO_EMMC_RST, 1);	
