...
{
	struct regulator *vreg_l2;
	int ret = 0;
	MM_INFO("%s %d\n", __func__, en);
	if (hsed_mic_en && !en) {
		MM_INFO("ignore disable mic-bias\n");
		return;
	}
	vreg_l2 = regulator_get(NULL, "xo_out");
	if (IS_ERR(vreg_l2)) {
		MM_ERR("%s: regulator_get(%s) failed (%ld)\n",
...
void flyer_rx_amp_enable(int en)
{
	if (curr_rx_mode != 0) {
		...
		MM_INFO("%s: curr_rx_mode 0x%x, en %d\n",
...
{
	bool xd03 = false;
	xd03 = gpio_get_value_cansleep(PM8058_GPIO_PM_TO_SYS(FLYER_NC_PMIC13)) == 0 &&
		gpio_get_value_cansleep(PM8058_GPIO_PM_TO_SYS(FLYER_NC_PMIC14)) == 1 && system_rev >= 3;
...
void __init flyer_audio_init(void)
{
	...
	if (gpio_get_value_cansleep(PM8058_GPIO_PM_TO_SYS(FLYER_NC_PMIC13)) == 0 && system_rev == 3)
	board_ver += 3;
	MM_INFO("%s board_ver = %d\n", __func__, board_ver);
