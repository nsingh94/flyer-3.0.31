...
static struct platform_device msm_proccomm_regulator_dev = {
	...
	.id   = -1,
	.dev  = {
		.platform_data = &flyer_proccomm_regulator_data
	}
};
#endif
...
static int sensor_power_enable(char *power, unsigned volt)
{
	...
	if (IS_ERR(vreg_gp)) {
		pr_err("[CAM] %s: regulator_get(%s) failed (%ld)\n",
			__func__, power, PTR_ERR(vreg_gp));
		return EIO;
	}

	rc = regulator_set_voltage(vreg_gp, volt*1000, volt*1000);
	if (rc) {
...
static unsigned int msm_marimba_setup_power(void)
{
	...
	if (rc) {
		printk(KERN_ERR "%s: regulator_enable() = %d \n",
					__func__, rc);
		goto out;
	}
...
static void msm_marimba_shutdown_power(void)
{
	...
	if (regulator_is_enabled(vreg_marimba_1))
	{
	...
	if (regulator_is_enabled(vreg_marimba_2))
	{
...
static int msm_hsusb_ldo_init(int init)
{
	...
	if (init) {
		...
		regulator_set_voltage(vreg_3p3, def_vol*1000, def_vol*1000);
	} else
...
static int flyer_battery_charging_ctrl(enum batt_ctl_t ctl)
{
	...
	case ENABLE_SLOW_CHG:
		if (1 <= engineerid) {
			...
			gpio_set_value_cansleep(PM8058_GPIO_PM_TO_SYS(FLYER_ADP_9V), 0);
	...
	case ENABLE_FAST_CHG:
		...
		if (system_rev >= XC) {
			result = gpio_direction_output(FLYER_GPIO_ISET_XC, 1);
			result = gpio_direction_output(FLYER_GPIO_MCHG_EN_N_XC, 0);
			gpio_set_value_cansleep(PM8058_GPIO_PM_TO_SYS(FLYER_ADP_9V), 1);
		} else {
...
/*
		int rc = regulator_set_voltage(qup_vreg, 1800*1000, 1800*1000);
		if (rc) {
...
struct regulator *vreg_s3;
struct regulator *vreg_mmc;
...
static uint32_t msm_sdcc_setup_vreg(int dev_id, unsigned int enable)
{
	...
	if (enable) {
		...
		rc = regulator_set_voltage(curr->vreg_data, curr->level*1000, curr->level*1000);
		if (rc) {
			printk(KERN_ERR "%s: regulator_set_voltage() = %d \n",
					__func__, rc);
		}
		rc = regulator_enable(curr->vreg_data);
		if (rc) {
			printk(KERN_ERR "%s: regulator_enable() = %d \n",
					__func__, rc);
		}
		...
		rc = regulator_disable(curr->vreg_data);
		if (rc) {
			printk(KERN_ERR "%s: regulator_disable() = %d \n",
					__func__, rc);
		}
	}
	return rc;
...
static void msm_sdc1_lvlshft_enable(void)
{
	...
	rc = regulator_set_voltage(vreg_mmc, 2850*1000, 2850*1000);
	if (rc)
		printk(KERN_ERR "%s: regulator_set_voltage() = %d \n",	__func__, rc);
...
static void __init msm7x30_init_mmc(void)
{
	...
	vreg_mmc = regulator_get(NULL, "gp10");
	if (IS_ERR(vreg_mmc)) {
		printk(KERN_ERR "%s: vreg get failed (%ld)\n",
...
*/
	...
	if (system_rev == 2) {/* for Led XC board */
		...
		if(gpio_get_value_cansleep(PM8058_GPIO_PM_TO_SYS(FLYER_PEN_LED3))){
	...
	if (system_rev >= 2) {
		...
		mdelay(1);
		if (gpio_get_value_cansleep(PM8058_GPIO_PM_TO_SYS(FLYER_NC_PMIC14)) == 0) {
