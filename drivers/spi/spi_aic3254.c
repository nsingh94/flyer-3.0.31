...
static int aic3254_config(CODEC_SPI_CMD *cmds, int size)
{
	...
	if (!codec_dev) {
		MM_ERR("%s: no spi device\n", __func__);
		return -EFAULT;
	}

	if (cmds == NULL) {
		MM_ERR("%s: invalid spi parameters\n", __func__);
		return -EINVAL;
	}
	...
	if (suspend_flag && ctl_ops->panel_sleep_in) {
		...
		suspend_flag = 0;
		if (ret < 0)
			MM_ERR("%s: cannot make panel awake,"
				"it might failed on transmit SPI command\n"
				, __func__);
		else
			MM_INFO("%s: success on invoking panel_sleep_in\n"
				, __func__);
	}
	...
	if (size < 1000) {
		for (i = 0; i < size; i++) {
			...
			case 'r':
				...
				if (retry <= 0)
					MM_INFO("3254 power down procedure"
...
static int aic3254_config_ex(CODEC_SPI_CMD *cmds, int size)
{
	...
	if (spi_t_cmds == NULL) {
		MM_ERR("%s: kmalloc spi transfer struct fail\n", __func__);
		goto error;
	} else
...
static void aic3254_powerdown(void)
{
	...
	if (aic3254_uplink != NULL) {
		MM_INFO("power off AIC3254 len(%d)++\n",
	...
	MM_INFO("power off AIC3254 %lldms --\n", t2);
	return;
}
...
static void aic3254_loopback(int mode)
{
	if (!(ctl_ops->lb_dsp_init &&
		...
		MM_INFO("%s: AIC3254 LOOPBACK not supported\n", __func__);
		return;
	}
	...
	MM_INFO("%s: set AIC3254 in LOOPBACK mode\n", __func__);
	switch (mode) {
	case 0:
		/* receiver v.s. imic */
	...
	case 13:
		...
		if (ctl_ops->lb_receiver_bmic)
			...
			MM_INFO("%s: receiver v.s. 2nd mic loopback not supported\n", __func__);
		break;
	...
	case 14:
		...
		if (ctl_ops->lb_speaker_bmic)
			...
			MM_INFO("%s: speaker v.s. 2nd mic loopback not supported\n", __func__);
		break;
	...
	case 15:
		...
		if (ctl_ops->lb_headset_bmic)
			...
			MM_INFO("%s: headset v.s. 2nd mic loopback not supported\n", __func__);
		break;
	default:
		break;
	}
...
{
	MM_INFO("%s: (%d,%d) uses 3254 default setting\n", __func__, path, en);
	if (en) {
		/* Downlink_Wakeup */
...
static int aic3254_set_config(int config_tbl, int idx, int en)
{
	...
	case AIC3254_CONFIG_MEDIA:
		...
		MM_INFO("%s: configure miniDSP index(%d) len = %d ++\n",
			__func__, idx, len);
		MM_INFO("%s: rx mode %d, tx mode %d\n",
		...
		MM_INFO("%s: configure miniDSP index(%d) time: %lldms --\n",
			__func__, idx, (t2));
		break;
	}
...
void aic3254_set_mode(int config, int mode)
{
	...
	case AIC3254_CONFIG_TX:
		/* TX */
		MM_INFO("%s: AIC3254_CONFIG_TX mode = %d\n",
			__func__, mode);
	...
	case AIC3254_CONFIG_RX:
		/* RX */
		MM_INFO("%s: AIC3254_CONFIG_RX mode = %d\n",
			__func__, mode);
	   ...
	   unsigned long argc)
{
	...
	if (aic3254_uplink == NULL ||
		...
		MM_ERR("%s: cmd 0x%x, invalid pointers\n", __func__, cmd);
		return -EFAULT;
	}
	...
	case AIC3254_SET_RX_PARAM:
		if (copy_from_user(&para, (void *)argc, sizeof(para))) {
			MM_ERR("%s: failed on copy_from_user\n", __func__);
			return -EFAULT;
		}

		MM_INFO("%s: parameters(%d, %d, %p)\n", __func__,
		...
		if (para.row_num > IO_CTL_ROW_MAX
			...
			MM_ERR("%s: data size mismatch with allocated"
					" memory (%d,%d)\n", __func__,
		...
		MM_INFO("%s: update table(%d,%d) successfully\n",
	...
	case AIC3254_CONFIG_VOLUME_L:
		...
		if (volume < -127 || volume > 48) {
			MM_ERR("%s: volume out of range\n", __func__);
			return -EFAULT;
		}

		MM_INFO("%s: AIC3254 config left volume %d\n",
				__func__, volume);
	...
	case AIC3254_CONFIG_VOLUME_R:
		...
		MM_INFO("%s: AIC3254 config right volume %d\n",
				__func__, volume);
	...
	case AIC3254_DUMP_PAGES:
		...
		if (i > AIC3254_MAX_PAGES) {
			MM_ERR("%s: invalid page number %d\n", __func__, i);
			return -EINVAL;
		}

		MM_INFO("========== %s: dump page %d ==========\n",
				__func__, i);
		...
		for (i = 0; i < AIC3254_MAX_REGS; i++) {
			...
			if (ret < 0)
				MM_ERR("read fail on register 0x%X\n", i);
			else
				MM_INFO("(0x%02X, 0x%02X)\n", i, data);
		}
		if (ctl_ops->rx_amp_enable)
			ctl_ops->rx_amp_enable(0);
		MM_INFO("=============================================\n");
	...
	case AIC3254_WRITE_REG:
		...
		MM_INFO("%s: command list (%c,%02X,%02X) (%c,%02X,%02X)\n",
	...
	case AIC3254_READ_REG:
		...
		if (copy_to_user((void *)argc, &reg, sizeof(CODEC_SPI_CMD)*2)) {
			MM_ERR("%s: failed on copy_to_user\n", __func__);
			return -EFAULT;
		}
		break;
	...
	case AIC3254_LOOPBACK:
		...
		MM_INFO("%s: index %d for LOOPBACK\n", __func__, i);
		aic3254_loopback(i);
		break;
	default:
		MM_ERR("%s: invalid command %d\n", __func__, _IOC_NR(cmd));
		ret = -EINVAL;
	}
...
static int __init spi_aic3254_init(void)
{
	...
	if (ret < 0) {
		MM_ERR("%s:failed to register spi driver(%d)\n", __func__, ret);
		return ret;
	}

	ret = misc_register(&aic3254_misc);
	if (ret < 0) {
		MM_ERR("%s:failed to register misc device\n", __func__);
	...
	if (IS_ERR(codec_drv->rx_mclk)) {
		MM_ERR("%s:failed to get mi2s mclk\n", __func__);
	...
	if (IS_ERR(codec_drv->rx_sclk)) {
		MM_ERR("%s:failed to get mi2s sclk\n", __func__);
		misc_deregister(&aic3254_misc);
		spi_unregister_driver(&spi_aic3254);
